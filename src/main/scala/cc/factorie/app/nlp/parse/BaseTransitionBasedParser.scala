...
class TransitionBasedParser2 extends BaseTransitionBasedParser {
...
object TransitionBasedParserTrainer2 extends cc.factorie.util.HyperparameterMain {
  def evaluateParameters(args: Array[String]) = {
    ...
    def testSingle(c: TransitionBasedParser2, ss: Seq[Sentence], extraText: String = ""): Unit = {
      if (ss.nonEmpty) {
        println(extraText + " " + c.testString(ss))
      }
    }

    def testAll(c: TransitionBasedParser2, extraText: String = ""): Unit = {
      println("\n")
    ...
    val c = new TransitionBasedParser2
    ...
    if (opts.saveModel.value) {
      ...
      val d = new TransitionBasedParser2
...
object TransitionBasedParserOptimizer2 {
